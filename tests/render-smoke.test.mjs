...
function installGlobals() {
  ...
  const mobius = {
    ...
    // Atlas binds window.mobius.createUseDocument(React) → a hook. Under the
    // stubbed no-op hooks this never actually subscribes; return a hook shape.
    createUseDocument: () => () => ({ data: null, setData: noop, durableWrite: async () => {}, online: true }),
  }
  try {
  ...
  // Atlas's load() calls the real global fetch on first render (useEffect is
